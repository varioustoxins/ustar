...
impl SASContentHandler for ComprehensiveTestHandler {
    ...
    fn start_saveframe(&mut self, line: usize, name: &str) -> bool { 
        self.output.push(format!("<start saveframe> [{}] {}", line, name));
        false 
    }
    
    fn end_saveframe(&mut self, line: usize, name: &str) -> bool { 
        self.output.push(format!("<end saveframe> [{}] {}", line, name));
        false 
    }
    ...
    fn data(&mut self, tag: &str, tagline: usize, value: &str, valline: usize, delimiter: &str, inloop: bool) -> bool {
        match delimiter {
            "\n" => {
                self.output.push(format!("<data> [t:{},v:{}] {} delimiter: {:?} inloop: {} value:", tagline, valline, tag, delimiter, inloop));
                for line in value.lines() {
                    self.output.push(format!("       {}", line));
                }
            }
            _ => {
                self.output.push(format!("<data> [t:{},v:{}] {} delimiter: {} inloop: {} value [multiline]: {}", tagline, valline, tag, delimiter, inloop, value));
            }
        }
        false
    }
...
fn test_simple_data_walker_output() {
    ...
    assert_eq!(handler.output.len(), expected_output.len(), 
               "Output length mismatch. Expected: {:?}, Got: {:?}", expected_output, handler.output);
    
    for (i, (expected, actual)) in expected_output.iter().zip(handler.output.iter()).enumerate() {
...
fn test_comprehensive_example_walker_output() {
    ...
    let expected_output_text = fs::read_to_string("tests/test_data/comprehensive_example_walker_output.txt")
    ...
    // Compare lengths
    assert_eq!(handler.output.len(), expected_lines.len(), 
               "Output length mismatch. Expected {} lines, got {} lines.\nFirst few actual lines: {:?}", 
               expected_lines.len(), handler.output.len(), handler.output.iter().take(10).collect::<Vec<_>>());
