...
fn main() {
    ...
    for file_path in &args[1..] {
        ...
        println!("UTF-8 BOM: {}", if has_bom { "✓ Present" } else { "✗ Not found" });
    ...
    for ch in content.chars() {
        ...
        if unicode_chars.is_empty() {
            ...
            println!("{:-<80}", "");
            println!("{:<8} {:<8} {:<8} {:<8} {:<20} {}", "Byte", "Line", "Column", "Char", "Unicode", "Description");
            println!("{:-<80}", "");
            ...
            for (byte_pos, line, col, ch) in unicode_chars.iter().take(50) {
                ...
                println!("{:<8} {:<8} {:<8} {:<8} U+{:04X} ({:<6}) {}", 
                    byte_pos, line, col, ch, *ch as u32, 
                    format!("'{}'", ch.escape_unicode()), unicode_name);
            }
            ...
            for (_, line_num, col, ch) in unicode_chars.iter().take(5) {
                if *line_num > 0 && *line_num <= lines.len() {
                    ...
                    println!("  {: <padding$}^--- '{}' (U+{:04X})", "", ch, *ch as u32, padding = padding);
