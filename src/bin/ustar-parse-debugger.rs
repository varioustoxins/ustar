...
use ustar::{parse, default_config, ErrorFormatMode, ConfigKey, ConfigValue};
...
fn main() {
    ...
    config.insert(ConfigKey::ErrorFormat, ConfigValue::ErrorFormat(ErrorFormatMode::Fancy));
        ...
        Err(e) => {
            let result = {
                ...
                match AsciiParser::parse(Rule::star_file, &content) {
                    Ok(_) => {
                        println!("Unexpected: direct pest parser succeeded where new system failed");
                        std::process::exit(0);
                    }
                    Err(pest_error) => {
                ...
                if let Some(result) = parse_result {
                    display_parse_debug_info(&content, error_line, &result, args.full_tree, !args.whitespace);
                    Some(result)
                } else {
...
fn display_parse_debug_info(content: &str, error_line: usize, result: &ParseResult, show_full_tree: bool, no_visible_whitespace: bool) {
    println!("   ✓ Found successful parse at byte position: {}\n", result.last_position);
    ...
    if remaining_lines.is_empty() {
        ...
        println!("    Showing unparsed content ({} lines):\n", total_lines_to_show);
...
fn print_tree_summary_with_indent(pair: &pest::iterators::Pair<Rule>, depth: usize, content: &str, indent_spaces: usize) {
    ...
    for (tree_line, text_line) in tree_lines.iter().zip(text_lines.iter()) {
        println!("{}{:<width$}  \"{}\"", indent, tree_line, text_line, width = max_tree_width);
    }
}
